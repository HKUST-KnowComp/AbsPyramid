    ...
    def _compute(
        ...
        if num_layers is None:
            if "deberta" in model_type:
                cur_model_type = "microsoft/" + model_type.split("/")[-1]
            else:
                cur_model_type = model_type
            num_layers = bert_score.utils.model2layers[cur_model_type]
        ...
        output_dict["precision"] = sum(output_dict["precision"]) / len(output_dict["precision"])
        output_dict["recall"] = sum(output_dict["recall"]) / len(output_dict["recall"])
        if output_dict["precision"] + output_dict["recall"] == 0:
            output_dict["f1"] = 0.0
        else:
            output_dict["f1"] = 2 * output_dict["precision"] * output_dict["recall"] / (output_dict["precision"] + output_dict["recall"])
        return output_dict
