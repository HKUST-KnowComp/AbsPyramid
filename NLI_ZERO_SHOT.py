...
def main():
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        probs = probs[:, entail_id]
        preds = np.array(probs >= 0.5, dtype=np.int64)
        labels = p.label_ids
        results = {}
    ...
    if training_args.do_eval:
        ...
        pred_label = torch.softmax(torch.tensor(pred_prob), dim=-1)
        pred_label = pred_label[:, entail_id]
        pred_label = np.array(pred_label >= 0.5, dtype=np.int64).reshape(-1, 1)
